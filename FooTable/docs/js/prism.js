...
// Plugin to make entity title show the real entity, idea by Roman Komarov
   ...
   * http://aahacreative.com/2012/07/31/php-syntax-highlighting-prism/ Modified
   * by Miles Johnson: http://milesj.me
   *
...
// Add HTML support of the markup language exists
  ...
  Prism.languages.scss = Prism.languages.extend('css', {
    ...
    // a selector must start at the end of a property or after a brace (end of
    // other rules or nesting) it can contain some caracters that aren't used
    // for defining rules or end of selector, & (parent selector), or
    // interpolated variable the end of a selector is found when there is no
    // rules in it ( {} or {\s}) or if there is a property (because an
    // interpolated var can "pass" as a selector- e.g: proper#{$erty}) this one
    // was ard to do, so please be careful if you edit this one :)
    'selector': /([^@;\{\}\(\)]?([^@;\{\}\(\)]|&amp;|\#\{\$[-_\w]+\})+)(?=\s*\{(\}|\s|[^\}]+(:|\{)[^\}]+))/gm
  });
...
// Hacks to fix eager tag matching finishing too early: <script src="<% Foo.Bar
// %>"> => <script src="<% Foo.Bar %>
  ...
  (function () {
    ...
    function highlightLines(pre, lines, classes) {
      ...
      for (var i = 0, range; range = ranges[i++];) {
        ...
        //if the line-numbers plugin is enabled, then there is no reason for
        // this plugin to display the line numbers
        ...
        if (hasClass(pre, 'line-numbers')) {
          //need to attack to pre as when line-numbers is enabled, the code tag
          // is relatively which screws up the positioning
  ...
  (function () {
    ...
    function getLanguage(tag) {
      ...
      // Lame way to detect MathML, but browsers donâ€™t expose interface names
      // there :(
