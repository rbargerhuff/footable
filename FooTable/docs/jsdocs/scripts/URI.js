...
(function (root, factory) {
  ...
  function URI(url, base) {
    ...
    this.href(url);
    // resolve to base according to
    // http://dvcs.w3.org/hg/url/raw-file/tip/Overview.html#constructor
    if (base !== undefined) {
  ...
  // ALPHA DIGIT "-" "." "_" "~" "!" "$" "&" "'" "(" ")" "*" "+" "," ";" "="
  // %encoded I've never seen a (non-IDN) hostname other than: ALPHA DIGIT . -
  ...
  URI.parseHost = function (string, parts) {
    ...
    if (string[0] === "[") {
      ...
      // I claim most client software breaks on IPv6 anyways. To simplify
      // things, URI only accepts IPv6+port in the format [2001:db8::1]:80 (for
      // the time being)
      ...
      // this notation is actually not allowed by RFC 3986, but we're a liberal
      // parser
  ...
  URI.buildQuery = function (data, duplicates) {
    // according to http://tools.ietf.org/html/rfc3986 or
    // http://labs.apache.org/webarch/uri/rfc/rfc3986.html being
    // »-._~!$&'()*+,;=:@/?« %HEX and alnum are allowed the RFC explicitly
    // states ?/foo being a valid use case, no mention of parameter syntax!
    // URI.js treats the query string as being
  ...
  URI.buildQueryParameter = function (name, value) {
    // http://www.w3.org/TR/REC-html40/interact/forms.html#form-content-type --
    // application/x-www-form-urlencoded don't append "=" for null values,
    // according to
    // http://dvcs.w3.org/hg/url/raw-file/tip/Overview.html#url-parameter-serialization
  ...
  URI.withinString = function (string, callback) {
    ...
    // best solution in a regex sprint we did a couple of ages ago at *
    // http://mathiasbynens.be/demo/url-regex *
    // http://rodneyrehm.de/t/url-regex.html
    return string.replace(URI.find_uri_expression, callback);
  };
  URI.ensureValidHostname = function (v) {
    // Theoretically URIs allow percent-encoding in Hostnames (according to RFC
    // 3986) they are not part of DNS and therefore ignored by URI.js
  ...
  p.readable = function () {
    var uri = this.clone();
    // removing username, password, because they shouldn't be displayed
