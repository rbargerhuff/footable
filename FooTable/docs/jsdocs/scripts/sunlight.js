...
(function (window, document, undefined) {
    ...
    return function (tokens) {
      ...
      for (j = 0; j < tokenRequirements.length; j++) {
        ...
        if (actual === undefined) {
          ...
          tokenIndexStart -= direction; //we need to reevaluate against this
                                        // token again
        }
        else {
          return false;
  ...
  function createBetweenRule(startIndex, opener, closer, caseInsensitive) {
    return function (tokens) {
      ...
      //check to the left: if we run into a closer or never run into an opener,
      // fail
  ...
  function defaultNumberParser(context) {
    ...
    //this will work for hex (0xef), octal (012), decimal and scientific
    // notation (1e3) anything else and you're on your own
  ...
  Highlighter.prototype = (function () {
    ...
    function getScopeReaderFunction(scope, tokenName) {
      ...
      //which means that we need to process the current char, rather than
      // peeking at the next
      return function (context, continuation, buffer, line, column, processCurrent) {
        ...
        if (!foundCloser) {
          //we need to signal to the context that this scope was never properly
          // closed this has significance for partial parses (e.g. for nested
          // languages)
    ...
    function tokenize(unhighlightedCode, language, partialContext, options) {
      ...
      //if continuation is given, then we need to pick up where we left off
      // from a previous parse basically it indicates that a scope was never
      // closed, so we need to continue that scope
      ...
      while (!context.reader.isEof()) {
        ...
        //flush default data if needed (in pretty much all languages this is
        // just whitespace)
        if (token !== null) {
          if (context.defaultData.text !== "") {
      ...
      var nodes = [],
        prepareText = function () {
          ...
          return function (token) {
            ...
            //tabstop madness: replace \t with the appropriate number of
            // characters, depending on the tabWidth option and its relative
    ...
    //partialContext allows us to perform a partial parse, and then pick up
    // where we left off at a later time this functionality enables nested
    // highlights (language within a language, e.g. PHP within HTML followed by
    // more PHP)
    ...
    return {
      ...
      highlightNode: function highlightRecursive(node) {
        ...
        //if the node is block level, we put it in a container, otherwise we
        // just leave it alone
